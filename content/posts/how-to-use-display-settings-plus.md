...
Also before this begins its important to understand that the there are two levels of current settings. The first would be the settings that **currently applied** and the other would be the **current custom** which are settings that are currently sitting inside the fields(whether they are applied or not).

# 1: Introduction
This blog will explain how to utilize the open-source display settings tool called "Display Settings Plus" to its full potential. First, I will walk you through how to download the software, then follow that up by explaining what each individual button does so you can use the software as good as it's creator.
...
# 3: How to Use
Now to jump into how this software is functions and should be used. To explain properly I will devide that app into 5 individual sections which then get cut into even tinier bite sized information for you to consume. Also don't be scared to not understand anything the first time you read it because you will likely understand after reading a more closely related section.
...
This portion allows the user to select (left-click) connected which monitor the following settings will affect/view. The highlighted bright pink indicates it has been selected or is being hovered on, while the grey indicates it is not in focus. The name is based on the physical port each monitor is plugged into.
...
The **position section** is pretty straightforward. This portion is a little redundant given the freehand position, but allows the user to pixel-push to get more finer grain control. Updating either the X or Y value will change the monitor's position in real time inside the freehand position section.
...
- Positions will be normalized when applied to ensure the farthest left monitor starts at X:0 and the highest monitor is Y:0. and offset all other positions to keep their relational offsets from eachother
...
The **reset button** in this section only resets the X and Y of the focused monitor to  what the software believe is the current applied settings (what they were after the last apply or what they were in the last state synced/pulled from the driver).
...
- **Inverted** – the defaulted top now faces down;
- **Right** – the defauled top now faces right;
...
In this section, you should notice your monitor(s) in their current custom settings version of thier position, rotation, and size (ratio). Each monitor displays a screenshot of what was on the screen when the application started, and the name corresponding to the physical port used. You can **left-click to drag monitors** or **right-click to pan the scene**.
...
- **Middle Piece**: Contains your saved presets which will be empty at first. Each preset appears as two buttons:
  - Clicking the **name** makes the button highlight pink indicating you've selected it and overwrites the current custom setting with it contained settings(if non-empty)
...
- **Modular Apply**: Applies each change one at a time with an option to undo certain parts. This is experimental and may cause incosistent results and looks spooky while apply each portion due to the dependency of each setting to look right.
...
- **Monitor Dropdown**: Lets the user select the primary monitor. The primary monitor is where apps will defaultly open and the taskbar will appear (if not shown on all monitors).
